...
void go_back_n(int sockfd, int windsz, int mtu, FILE* in_file, const struct sockaddr* dest_addr, socklen_t servlen) {
    ...
    setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &read_timeout, sizeof(read_timeout)); // add recv timeout on socket so its non blocking
    ...
    while (fgets(buffer, mtu - HEADER_SIZE, in_file) != NULL) {
        ...
        if (recv_bytes != -1) {
            sscanf(recv_buffer, "%i %s", ACKnum, response); // parse for ack num and reponse
        ...
        if(alarm_bool) { //if timeout is triggered, resend window
            int temp = send_base;
